...
module.exports = async (req, res, next) => {
    try {
       ...
       // logger.log("info", `intent:  ${JSON.stringify(req.body)}`, null);

        let fulfillment = new dialogflowFullfillment(config.fullfillmentConfig, req.body);
        if (intentMapper[requestIntent]) {
            await intentMapper[requestIntent](fulfillment,parameters);
        } else {
            const requiredIntent = getIntent(requestIntent);
            await require(requiredIntent)(fulfillment,parameters);
        }
